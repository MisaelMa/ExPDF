defmodule Pdf.Case do
  ...
  def export(%Pdf.Document{current: page}), do: export(page)
  ...
  @doc """
  Check if the current page stream contains the given string.
  Works with both Page and Document structs.
  """
  def pdf_stream_contains?(%Pdf.Document{} = doc, needle) do
    export(doc) =~ needle
  end

  def pdf_stream_contains?(%{stream: _} = page, needle) do
    export(page) =~ needle
  end

  @doc """
  Create a new document with sensible defaults for testing.
  """
  def new_test_doc(opts \\ []) do
    defaults = [size: :a4, compress: false]
    merged = Keyword.merge(defaults, opts)
    Pdf.new(merged) |> Pdf.set_font("Helvetica", 12)
  end
end
